  ...
  const onSubmit = (data: RegisterFormValues) => {
    ...
    registerMutation.mutate(formData, {
      onSuccess: (userData) => {
        ...
        console.log("Регистрация успешна, перенаправляем напрямую на основную страницу");
        
        // Прекращаем делать проверку сессии, поскольку она может не работать в контейнерной среде
        // Вместо этого просто перенаправляем на начальную страницу приложения
        // Поскольку мы знаем, что регистрация прошла успешно (сервер вернул код 201)
        
        // Обработчик для перенаправления на гороскоп (первая доступная страница)
        const redirect = () => {
          console.log("Прямое перенаправление на страницу гороскопа");
          // Используем /horoscope вместо /home, потому что это может быть первый доступный маршрут
